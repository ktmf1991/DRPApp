...
server <- function(input, output, session) {
  ...
  observeEvent(input$do, {
    ...
    ed2 <- lapply(seq_along(ed), function(y) cbind(names(ll4.res[[y]]),rep(names(ed)[y],nrow(ed[[y]])),ed[[y]]))
    ed2 <- lapply(ed2, function(x) {
      dat <- x
      colnames(dat) <- c("Drug","PP",paste0(rep(paste0("EC",c(10,25,50,75,90)),each = 2),c("","_SE")))
      return(dat)
    })
    ed2 <- lapply(ed2,function(x){
                                  pp <- str_split(x$PP,"\\.",simplify = T)
                                  colnames(pp) <- c("Patient","Plate")
                                  return(cbind(x[,-2],pp))
                                })
    ed2 <- odlc(ed2)
    ...
    ic2 <- lapply(seq_along(ic), function(y) cbind(names(ll4.res[[y]]),rep(names(ic)[y],nrow(ic[[y]])),ic[[y]]))
    ic2 <- lapply(ic2, function(x) {
      dat <- x
      colnames(dat) <- c("Drug","PP",paste0(rep(paste0("IC",c(10,25,50,75,90)),each = 2),c("","_SE")))
      return(dat)
    })
    ic2 <- lapply(ic2,function(x){
      pp <- str_split(x$PP,"\\.",simplify = T)
    ...
    ed <- merge.list(ed, by = "Drug",all = TRUE)
    ...
    ic <- merge.list(ic, by = "Drug", all = T)
